...
class FlatBreakout(DetectorBase):
    ...
    def scan(self, df: pd.DataFrame, symbol: str = "") -> list[Event]:
        ...
        n = len(df)
        events = []
        for s in range(130, n - self.min_len - 2):
            for length in (self.min_len, self.min_len + 10, self.max_len):
                ...
                if rng > self.max_range_pct:
                    continue
                # 平台内缩量回调出现过
                if not (vol_ma5.iloc[s:e0] < 0.8 * vol_ma20.iloc[s:e0]).any():
                    continue
                # 平台前上涨段 >=25%（120日回看）
                pre = C.iloc[e0 - 1] / C.iloc[max(0, e0 - 121):e0 - 1].min() - 1
                if pre < 0.25:
                    continue
                # 平台内回撤不超过前段涨幅25%
                if float(seg["L"].min()) / C.iloc[max(0, s - 60):s].max() - 1 < -0.25:
                    continue
                upper = hi * 1.001 + 0.1 * (hi - lo) * 0.5
                # 确认窗口：平台结束后 15 个交易日内首次放量收盘上穿
                for t in range(e0, min(e0 + 15, n)):
                    if C.iloc[t] <= upper:
                        continue
...
class CupHandle(DetectorBase):
    ...
    def scan(self, df: pd.DataFrame, symbol: str = "") -> list[Event]:
        ...
        for i in range(self.min_cup_days + 2 * k + 20, n):
            t = df.index[i]
            vis = pv.as_of(t)
            ...
            if i - pos_rim < k or i - pos_rim > 25:
                continue  # 右沿之后需在柄区
            H_top = float(df.loc[rim_row["pivot_date"], "H"])
            # 左侧涨幅：H_top 前 20~120 日内最低点起涨 >=30%
            lo_win = df["L"].iloc[max(0, pos_rim - 120):max(1, pos_rim - 19)]
            if lo_win.empty:
                continue
            ...
            # 柄区：右沿之后至今，回调深度<=杯深一半且<=12%
            handle = df.iloc[pos_rim:i]
            if len(handle) < 5 or len(handle) > 21:
                continue
            h_low = float(handle["L"].min())
            h_depth = (H_top - h_low) / H_top
            if h_depth > depth / 2 or h_depth > 0.12:
                continue
            ...
            if df["C"].iloc[i] > buy_pt:
                ...
                if (vr is not None and vr >= 2.0 and v_handle_base > 0
                        and float(df["V"].iloc[i]) >= 1.4 * v_handle_base):
                    events.append(self._event(
                        df, "confirmed", df.index[max(0, cup_win_start)], t,
                        t, t,
                        key_levels={"buy_point": round(buy_pt, 3),
                                    "stop": round(buy_pt * 0.93, 3)},
    ...
    def scan(self, df: pd.DataFrame, symbol: str = "") -> list[Event]:
        ...
        for i in range(70, n):
            ...
            if not (c > o and c / prev_c - 1 >= self.min_gain):
                continue
            # 核心条件：当日量 > 前10日所有下跌日最大量
            down_vols = [float(df["V"].iloc[j]) for j in range(i - 10, i)
                         if float(df["C"].iloc[j]) < float(df["C"].iloc[j - 1])]
            if not down_vols:
                continue
            if float(df["V"].iloc[i]) <= max(down_vols):
                continue
            # 收盘接近日内高点：(H-C)/(H-L)<=0.4
            if h > l and (h - c) / (h - l) > 0.4:
                continue
            # 前提过滤：C>MA50；距MA10<=5%
            ...
            if pd.isna(m10) or abs(c / m10 - 1) > self.max_ext_ma10:
                continue
            # 蓄势前提：此前15~65日区间振幅<=25% 或上升趋势中回调<=15%
            win = df["C"].iloc[i - 65:i]
            rng = float(win.max()) / float(win.min()) - 1
            pullback = 1 - c / float(df["C"].iloc[i - 20:i].max())
            if not (rng <= 0.25 or (c > m50 and pullback <= 0.15)):
                continue
            events.append(self._event(
                df, "confirmed", df.index[i - 10], df.index[i],
